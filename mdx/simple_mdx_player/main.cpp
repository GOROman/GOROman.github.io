...
//  printf("file loaded %d\n", size);
  ...
  WGETARG* a = (WGETARG*)arg;
  a->data = (uint8_t*)malloc(size);
  memcpy(a->data, buffer, size);
  a->size = size;
  g_wget_flag = true;
}
	...
	uint32_t *sizeRet)
{
	...
	*sizeRet = size;
	...
	g_wget_flag = false;
	WGETARG wgetarg;
	emscripten_async_wget_data(fileName, &wgetarg, onLoadedData, onErrorData );

	int retry = 100;
	while(g_wget_flag == false) {
		emscripten_sleep(100);	//10ms
		retry --;
		if ( retry <0 ) {
			break;
		}
	}
	*sizeRet = wgetarg.size;
	return wgetarg.data;
#endif
}
...
//	https://ftp.modland.com/pub/modules/MDX/-%20unknown/Genocide/st1-1.mdx
...
/*
        ...
        size_t  mybufsz = sizeof(str_out);

        ic = iconv_open("SJIS", "UTF8");	// from, to
